...
for k,v in params.items():
    ...
    for i in range(len(params[k])):
        if type(params[k][i]) == np.float64: # dealing with single values
            saved_val = params[k][i] 
            params[k][i] = saved_val + eps

            #params_plus_eps[k][i] += eps
            ...
            central_diff = (loss_plus - loss_minus)/(2*eps)
            params['grad_' + k][i]= central_diff
            params[k][i] = saved_val
        else:
            for j in range(len(params[k][i])): # dealing with arrays
                saved_val = params[k][i][j] 
                params[k][i][j] = saved_val + eps
                h1_plus = forward(x,params,'layer1')
                probs_plus = forward(h1_plus,params,'output',softmax)
                loss_plus, acc_plus = compute_loss_and_acc(y, probs_plus)
