...
parser.add_argument('--tolerance', type=float, default=.7, help='binary threshold of intensity difference when computing the mask')
...
for i in range(1, seq.shape[2]):
    ...
    # create a three channel image for It1
    ...
    if i == 1 or i == 2 or i == 30 or i == 60 or i == 90 or i == 120:
        # fig = plt.subplot(1,2,1)
        # im = plt.imshow(It, cmap='gray')
        fig = plt.figure()
        im = plt.imshow(channeled_img, cmap='gray')
        #plt.savefig('../result/antseqInverse_' + str(i) + '.png')
        plt.show() 
