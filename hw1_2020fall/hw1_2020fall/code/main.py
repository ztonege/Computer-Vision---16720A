...
def main():
    ...
    # #hist_SPM = visual_recog.get_feature_from_wordmap_SPM(opts, wordmap)
    ...
    # ## Q2.5
    # trained_system = np.load(join(opts.out_dir, 'trained_system.npz'))
    # n_cpu = util.get_num_CPU()
    # conf, accuracy = visual_recog.evaluate_recognition_system(opts, n_worker=n_cpu)
    
    # print(conf)
    # print(accuracy)
    # np.savetxt(join(opts.out_dir, 'confmat9.csv'), conf, fmt='%d', delimiter=',')
    # np.savetxt(join(opts.out_dir, 'accuracy9.txt'), [accuracy], fmt='%g')
